...
import * as vinyl from './polyvinyl.js'
...
const babelTransformCode = (options: TransformOptions) => (code: string) =>
...
export const replaceNBSP = cond([
  [
    testHTML$JS$JSX,
    partial(vinyl.transformContents, (contents: string) =>
...
function tryTransform(wrap: any) {
  return function transformWrappedPoly(source: string) {
...
const babelTransformer = (preview = false) => {
  const babelTransform = babelTransformCode(
    preview ? babelOptionsJSPreview : babelOptionsJS
  )
...
export const composeHTML = cond([
  [
    testHTML,
    flow(
      partial(vinyl.transformHeadTailAndContents, (source: string) => {
        const div = document.createElement('div')
